...
function printTable(fila,columna){
...
printTable(tableroMod.length,tableroMod[0].length);

document.querySelector("table").addEventListener("click", (e)=>{
    let target = e.target;
    let columna = target.cellIndex;
    let fila = target.parentElement.rowIndex;
    console.log("fila:" + fila + "columna:" + columna);
    if (tableroMod[fila][columna].minada) {
        tableView.rows[fila].cells[columna].textContent = "M";
    } else {
    tableView.rows[fila].cells[columna].textContent = tableroMod[fila][columna].numero;
    }
})
